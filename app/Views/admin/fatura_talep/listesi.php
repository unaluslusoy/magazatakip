<?php
$title  ="<h2>Fatura Talepleri Listesi</h2>";
$link   = "Fatura Talepleri Listesi";
                        ...
                        <?php foreach ($faturaTalepleri as $talep): ?>
                            <tr>
                                ...
                                <td class="text-end">
                                    ...
                                    <!--begin::Menu-->
                                    <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-lg-425px py-4" data-kt-menu="true">
                                        <!--begin::Menu item-->

                                        <div class="menu-item px-3">
                                            <a href="/admin/fatura_talep/duzenle/<?php echo htmlspecialchars($talep['id']); ?>" class="menu-link px-3">Düzenle</a>
                                        </div>
                                        <div class="menu-item px-3">
                                            <a class="menu-link px-3 send-whatsapp" data-id="<?= htmlspecialchars($talep['id']); ?>">WhatsApp ile Gönder</a>
                                        </div>
                                        <!--end::Menu item-->
                                        <form class="menu-item px-3" action="/admin/fatura_talep/sil/<?= $talep['id'] ?>" method="post" onsubmit="return confirmDelete()">
                                            <a type="submit" class="menu-link px-3">Sil</a>
                                        </form>
...
<script>
    function confirmDelete() {
        return confirm('Bu işlemi gerçekleştirmek istediğinizden emin misiniz? Silme işlemi geri alınamaz.');
    }
    document.querySelectorAll('.send-whatsapp').forEach(button => {
        button.addEventListener('click', function() {
            const faturaId = this.getAttribute('data-id');
            fetch(`/admin/fatura_talep/getFaturaDetails/${faturaId}`)
                .then(response => response.json())
                .then(data => {
                    if (data.success) {
                        const musteriTelefon = data.fatura.musteri_telefon;
                        const faturaPdf = data.fatura.fatura_pdf_path;
                        const musteriAd = data.fatura.musteri_ad;

                        if (musteriTelefon && faturaPdf) {
                            fetch('/admin/fatura_talep/sendWhatsapp', {
                                method: 'POST',
                                headers: {
                            ...
                            alert('Müşteri telefonu veya fatura PDF eksik.');
                        }
                    } else {
