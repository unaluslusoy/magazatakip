...
-- Kuyruk (job_queue) ek indeksler
SET @def := IF(EXISTS(SELECT 1 FROM INFORMATION_SCHEMA.STATISTICS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME='job_queue' AND INDEX_NAME='idx_worker_status'), 'SELECT 1', 'CREATE INDEX idx_worker_status ON job_queue (worker_id, status)');
PREPARE s12 FROM @def; EXECUTE s12; DEALLOCATE PREPARE s12;

SET @def := IF(EXISTS(SELECT 1 FROM INFORMATION_SCHEMA.STATISTICS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME='job_queue' AND INDEX_NAME='idx_reserved_at'), 'SELECT 1', 'CREATE INDEX idx_reserved_at ON job_queue (reserved_at)');
PREPARE s13 FROM @def; EXECUTE s13; DEALLOCATE PREPARE s13;
