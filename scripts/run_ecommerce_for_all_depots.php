...
$lockFile = sys_get_temp_dir() . '/run_ecommerce_all_depots.lock';
...
try {
	...
	$db = $svc->repoDb();
	$date = $argv[1] ?? null; // opsiyonel tarih
	$rows = $db->query("SELECT depo_id AS id FROM tamsoft_depolar ORDER BY depo_id ASC")->fetchAll(PDO::FETCH_ASSOC) ?: [];
	$results = [];
	foreach ($rows as $r) {
		$id = (int)($r['id'] ?? 0);
		if ($id <= 0) { continue; }
