...
$lockFile = sys_get_temp_dir() . '/tamsoft_ecommerce_cron.lock';
...
try {
    ...
    // İsteğe bağlı: CLI argümanları ile tarih ve depo id verilebilir
    // Kullanım: php tamsoft_ecommerce_cron.php 2024-01-01 3
    $date = $argv[1] ?? null;
    $depoId = isset($argv[2]) ? (int)$argv[2] : null;
    $res = $svc->refreshDepotQtyFromEcommerce($date, $depoId);
    $payload = ['ok'=>true] + $res;
